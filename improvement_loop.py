...
def collapse_duplicate_sections(markdown: str) -> str:
    ...
    for section in REQUIRED_ISSUE_SECTIONS:
        ...
        if not bodies:
            ...
            continue
        if section in {"## Workflow", "## CTA", "## Sources", "## Tool of the Week"}:
            combined: list[str] = []
            seen: set[str] = set()
            for body in bodies:
...
def ensure_affiliate_disclosure(markdown: str) -> str:
    if not has_affiliate_reference(markdown) or has_affiliate_disclosure(markdown):
        return markdown
    body = section_body(markdown, "## Tool of the Week")
    disclosure = (
        "\n\nDisclosure: ForgeCore may earn a commission if you buy through partner links, "
        "but recommendations are based on workflow fit, not payout."
    )
    return replace_section(markdown, "## Tool of the Week", (body + disclosure).strip())
...
def ensure_cta(markdown: str) -> str:
    ...
    if REQUIRED_CTA_URL and REQUIRED_CTA_URL not in body:
...
def enforce_deterministic_guardrails(markdown: str) -> tuple[str, list[str]]:
    ...
    updates = [
        ...
        ("affiliate disclosure", ensure_affiliate_disclosure),
...
def lesson_rule_from_item(item: str) -> str:
    ...
    if "affiliate" in lower or "partner" in lower or "commission" in lower:
        return "Any affiliate or partner-link language must include a clear commission disclosure."
...
def persist_editorial_lessons(issue_path: Path, critic: dict[str, Any] | None, gate: dict[str, Any] | None, changed: bool, reason: str) -> None:
    priorities = (extract_critic_items(critic) + extract_gate_errors(gate) + extract_gate_warnings(gate))[:MAX_TARGETED_REWRITE_ITEMS]
    if not priorities and not changed:
        return
    lines = [f"## {now_str()} — {issue_path.as_posix()}", f"- Improvement result: {'changed' if changed else 'unchanged'} ({reason})"]
    if critic:
...
def call_editor(prompt: str) -> str:
    client = OpenAI()
    result = client.chat.completions.create(model=EDITOR_MODEL, messages=[{"role": "user", "content": prompt}], temperature=0.2)
...
def build_prompt(issue_path: Path, issue_text: str, critic: dict[str, Any] | None, gate: dict[str, Any] | None) -> str:
    priorities = extract_critic_items(critic) + extract_gate_errors(gate) + extract_gate_warnings(gate)
    ...
    return (
        ...
        + f"The subscribe CTA URL is {REQUIRED_CTA_URL}. Do not use Beehiiv as the ForgeCore signup destination unless that exact URL is provided.\n"
        + "If you mention affiliate, partner, commission, referral, or sponsored-link language, include a clear Disclosure sentence.\n\n"
        + "## Must-fix priorities\n"
        + priority_block
