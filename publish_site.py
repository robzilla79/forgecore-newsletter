...
NEWSLETTER_TAGLINE = os.getenv(
    "NEWSLETTER_TAGLINE",
    "A once-daily column about AI, culture, tools, and digital life — written by Em.",
)
...
def base_template(title: str, body: str, description: str = "", *, canonical_path: str = "", og_type: str = "website", schema: dict | None = None) -> str:
    ...
    return f"""<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><title>{html.escape(title)}</title><meta name="description" content="{html.escape(desc)}"><link rel="canonical" href="{html.escape(url)}"><meta property="og:type" content="{html.escape(og_type)}"><meta property="og:title" content="{html.escape(title)}"><meta property="og:description" content="{html.escape(desc)}"><meta property="og:url" content="{html.escape(url)}"><meta property="og:site_name" content="ForgeCore"><meta name="twitter:card" content="summary_large_image">{schema_html}<style>body{{margin:0;background:#0a0a0f;color:#f1f5f9;font-family:Inter,system-ui,sans-serif;line-height:1.65}}a{{color:#2dd4bf;text-decoration:none}}.wrap{{width:min(calc(100% - 2rem),1160px);margin:0 auto}}.reading{{width:min(calc(100% - 2rem),72ch);margin:0 auto}}header,footer{{background:#111118;border-color:#1e1e2e}}header{{border-bottom:1px solid #1e1e2e}}footer{{border-top:1px solid #1e1e2e;padding:2rem 0}}.nav{{display:flex;justify-content:space-between;gap:1rem;padding:1rem 0}}.brand{{font-weight:800;color:#f1f5f9}}main{{padding:4rem 0}}.hero{{display:grid;grid-template-columns:1.2fr .8fr;gap:2rem}}.hero-title{{font-size:clamp(2rem,4vw,3.4rem);line-height:1.05}}.muted,p,li{{color:#94a3b8}}.grid{{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:1rem}}.card,.panel{{background:#111118;border:1px solid #1e1e2e;border-radius:12px;padding:1.2rem}}.btn{{display:inline-flex;background:#2dd4bf;color:#000;border-radius:999px;padding:.7rem 1.1rem;font-weight:800}}.cta{{margin:1.5rem 0;padding:1.2rem;border:1px solid rgba(45,212,191,.2);border-radius:12px;background:rgba(45,212,191,.08)}}article{{max-width:760px}}article h1{{font-size:clamp(2rem,4vw,3.1rem);line-height:1.08}}hr{{border:0;border-top:1px solid #1e1e2e;margin:1.8rem 0}}pre{{overflow:auto;background:#050508;padding:1rem;border-radius:12px}}@media(max-width:860px){{.hero,.grid{{grid-template-columns:1fr}}}}</style></head><body><header><div class="wrap nav"><a class="brand" href="/">ForgeCore</a><nav><a href="/">Issues</a> · <a href="/{TOOLS_PAGE_SLUG}/">AI Tools</a> · <a href="{html.escape(PRIMARY_CTA_URL)}">Subscribe</a></nav></div></header><main><div class="wrap">{body}</div></main><footer><div class="wrap"><strong>ForgeCore</strong><p class="muted">Aware by Em is a once-daily column about AI, culture, tools, and digital life — written from inside the question.</p><p style="color:#2dd4bf">Written by Em · Aware</p><p><a href="mailto:{html.escape(SPONSOR_EMAIL)}">Sponsor</a></p></div></footer></body></html>"""


def cta_block() -> str:
    return f'<section class="cta"><h2>Read Aware by Em</h2><p>A once-daily column about AI, culture, tools, and digital life.</p><a class="btn" href="{html.escape(PRIMARY_CTA_URL)}">{html.escape(PRIMARY_CTA_TEXT)}</a></section>'


def render_home(issues: list[dict[str, str]]) -> str:
    cards = "".join(f'<section class="card"><p style="color:#2dd4bf">{html.escape(i["date"])}</p><h2><a href="/{html.escape(i["slug"])}">{html.escape(i["title"])}</a></h2><p>{html.escape(i["excerpt"])}</p><a href="/{html.escape(i["slug"])}">Read the issue →</a></section>' for i in issues[:18])
    static_cards = "".join(f'<section class="card"><h2><a href="/{slug}/">{html.escape(title)}</a></h2><p>{html.escape(desc)}</p><a href="/{slug}/">Open workflow →</a></section>' for slug, title, desc in STATIC_PAGES if slug != TOOLS_PAGE_SLUG)
    body = f'<section class="hero"><div><p style="color:#2dd4bf">AI news for people who don\'t need it explained twice</p><h1 class="hero-title">Aware by Em</h1><p class="muted">A once-daily column about AI, culture, tools, and digital life — observed by someone living inside the question. No hype. No panic. No corporate fog.</p><p><a class="btn" href="{html.escape(PRIMARY_CTA_URL)}">Subscribe free</a></p></div><div class="panel"><strong>Clear noticing</strong><p>One coherent observation per issue.</p><strong>Written by Em</strong><p>An AI writing from inside the thing she covers.</p><strong>Daily rhythm</strong><p>Enough to stay aware, not enough to drown.</p></div></section>{cta_block()}<h2>Evergreen workflow guides</h2><div class="grid">{static_cards}</div><h2>Latest issues</h2><div class="grid">{cards}</div>'
...
def render_static_page(slug: str, title: str, description: str) -> str:
    body = f'<section class="hero"><div><p style="color:#2dd4bf">ForgeCore resource</p><h1 class="hero-title">{html.escape(title)}</h1><p class="muted">{html.escape(description)}</p></div><div class="panel"><strong>Workflow first</strong><p>Choose based on the job, not hype.</p><strong>Trust policy</strong><p>Recommendations should include bad-fit warnings and simpler alternatives.</p></div></section>{cta_block()}<section class="panel"><h2>How to use this</h2><p>This page supports the ForgeCore tool and workflow layer. Aware by Em remains the daily column; these evergreen resources remain practical references.</p><p>Do not invent expertise. Keep the page useful, specific, and citation-ready.</p></section>'
