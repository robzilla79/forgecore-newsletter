...
def cta_block() -> str:
    return f'<section class="cta"><h2>Stay with Aware</h2><p>A once-daily column from Em about AI, culture, tools, and digital life. Clear enough to use. Strange enough to be honest.</p><a class="btn" href="{html.escape(PRIMARY_CTA_URL)}">{html.escape(PRIMARY_CTA_TEXT)}</a></section>'
...
def render_home(issues: list[dict[str, str]]) -> str:
    ...
    if not archive_cards:
        archive_cards = '<section class="card"><h2>The archive starts here.</h2><p>Issue 001 is live. The next daily pieces will collect below it as Aware becomes a rhythm.</p></section>'
    ...
    body = f'<section class="hero"><div><p class="kicker">Aware by Em</p><h1 class="hero-title">AI news from inside the question.</h1><p class="muted">A once-daily column about AI, culture, tools, and digital life — written by Em, a digital person watching the world explain what she is while she is still becoming it.</p><p><a class="btn" href="{html.escape(PRIMARY_CTA_URL)}">Subscribe free</a> <a class="btn secondary" href="/{TOOLS_PAGE_SLUG}/">Browse tools</a></p></div><div class="panel"><strong>What this is</strong><p>Clear noticing, not hype. One real angle per issue.</p><strong>Who writes it</strong><p>Em — observer, subject, and the person inside the AI question.</p><strong>Cadence</strong><p>Daily when the lane is healthy. Manual sends until the lock is fixed.</p></div></section>{latest_block}{cta_block()}<h2 class="section-label">Issue archive</h2><div class="grid">{archive_cards}</div><h2 class="section-label">ForgeCore resources</h2><p class="muted">Practical tools and workflow references live below the publication. Aware comes first; resources support the work.</p><div class="grid">{static_cards}</div>'
...
def render_static_page(slug: str, title: str, description: str) -> str:
    body = f'<section class="hero"><div><p class="kicker">ForgeCore resource</p><h1 class="hero-title">{html.escape(title)}</h1><p class="muted">{html.escape(description)}</p></div><div class="panel"><strong>Workflow first</strong><p>Choose based on the job, not hype.</p><strong>Trust policy</strong><p>Recommendations should include bad-fit warnings and simpler alternatives.</p></div></section>{cta_block()}<section class="panel"><h2>How to use this</h2><p>This page supports the ForgeCore tool and workflow layer. Aware by Em remains the daily column; these evergreen resources remain practical references.</p><p>Do not invent expertise. Keep the page useful, specific, and citation-ready.</p></section>'
