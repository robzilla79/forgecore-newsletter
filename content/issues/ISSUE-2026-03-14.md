# Local AI Advancements: Ollama's MLX-Powered Performance and Simplified Workflows
...
## Top Story
Ollama's updates are not just about one new feature; they're about several aligning around the same idea: making local and hybrid AI deployment feel natural for working teams, not just enthusiasts. The first piece is performance, followed by setup reduction, coding workflow support, and image generation.
...
## Highlights
- Ollama's MLX preview is a bet on Apple Silicon as a serious local AI workstation path.
- `ollama launch` reduces setup drag and makes local or hybrid starts easier to standardize.
- Claude Code support gives coding teams another way to work with local models instead of remote-only defaults.
- Codex support expands the stack from chat to code-reading, modification, and execution workflows.
- Image generation broadens the platform from developer tooling into creative and content workflows.
